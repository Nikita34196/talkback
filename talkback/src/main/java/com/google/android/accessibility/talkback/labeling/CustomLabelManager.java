...
public class CustomLabelManager extends TalkBackLabelManager {
  ...
  public boolean canAddLabel(@Nullable AccessibilityNodeInfoCompat node) {
    ...
    @Nullable Pair<String, String> parsedId = splitResourceName(resourceName);
    if (parsedId == null || parsedId.first == null) {
      return false;
    }

    // For now, do not allow labeling views that have a different resource namespace than their
    // parent application.  Label-database needs redesign to store such labels.
    try {
  ...
  public boolean setLabel(@Nullable AccessibilityNodeInfoCompat node, @Nullable String userLabel) {
    ...
    Label oldLabel = getLabelForViewIdFromCache(resourceName);
    if (oldLabel == null) {
      addLabel(resourceName, userLabel);
    } else if (userLabel == null) {
      removeLabel(oldLabel);
    } else {
      oldLabel.setText(userLabel);
      oldLabel.setTimestamp(System.currentTimeMillis());
